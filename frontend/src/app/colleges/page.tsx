...
export default function CollegeListingPage() {
  ...
  const [maxFees, setMaxFees] = useState(600000); // 6L max budget
  const [page, setPage] = useState(1);
  const [totalPages, setTotalPages] = useState(1);
  const limit = 6; // 6 cards per page

  // Trigger search update when query param changes
  useEffect(() => {
  ...
  useEffect(() => {
    const delayDebounce = setTimeout(() => {
      fetchColleges();
    }, 300); // Debounce API calls for input sliders/typing
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        {/* Left Side: Advanced Filters (Desktop Sidebar) */}
